...
void processCommand(ClientSession* s, const string& line) {
    ...
    if (cmd == "QUIT") {
        ...
        string new_path = (arg.front() == '/') ? arg : s->current_dir + "/" + arg;
        char resolved[PATH_MAX];
        if (realpath(new_path.c_str(), resolved) == nullptr) {
            sendResponse(s->fd, 550, "未找到目录");
            return;
        }
        string real_path = resolved;
        if (real_path.find(DIR_ROOT) != 0) {
            sendResponse(s->fd, 550, "路径错误");
            return;
        }
        struct stat st;
        if (stat(real_path.c_str(), &st) == -1 || !S_ISDIR(st.st_mode)) {
            sendResponse(s->fd, 550, "不是目录");
            return;
        }
        s->current_dir = real_path;
        ...
        string fullpath = s->current_dir + "/" + arg;
        ...
        if (s->restart_pos > 0)
            lseek(fd, s->restart_pos, SEEK_SET);
        ...
        releasePassivePort(s->data_port);
        s->data_listen_fd = -1;
        sendResponse(s->fd, error ? 451 : 226, error ? "下载失败" : "下载成功");
        s->restart_pos = 0;
    }
    else {
...
int main(int argc, char* argv[]) {
    ...
    if (argc >= 4) {
        ...
        if (ftp_port <= 0 || ftp_port > 65535) ftp_port = DEFAULT_FTP_PORT;
    }
